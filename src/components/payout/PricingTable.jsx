...
import {apiService} from '../../services/authService'; // Assuming apiService is set up for API requests
...
export default function PricingTable() {
  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      try {
        const response = await apiService.get('/user'); // Replace with the actual endpoint
        const { username, email } = response.data.user.auth_user;
  ...
  const sendPlanToCheckout = async (plan) => {
    ...
    try {
      const response = await apiService.post('/checkout', data); // Using apiService.post
  ...
  const handleChatbotIndividual = () => {
    sendPlanToCheckout('ChatbotIndividual');
  };

  const handleChatbotPro = () => {
    sendPlanToCheckout('ChatbotPro');
  };
        ...
        <div className="grid max-w-4xl grid-cols-1 mx-auto mt-8 text-center sm:grid-cols-2 sm:mt-16 gap-y-5 gap-x-10">
          ...
          <div className="overflow-hidden bg-white border border-gray-200 rounded-2xl">
            <div className="p-6 md:p-10">
              ...
              <p className="mt-8 text-xl font-bold text-gray-900">19.99 €</p>
              ...
              <button
                onClick={handleChatbotIndividual}
                className="
          ...
          <div className="overflow-hidden bg-white border border-gray-200 rounded-2xl">
            <div className="p-6 md:p-10">
              ...
              <p className="mt-8 text-xl font-bold text-gray-900">99.99 €</p>
              ...
              <button
                onClick={handleChatbotPro}
                className="
                  inline-flex items-center justify-center px-8 py-3.5 mt-10 text-base font-bold text-white 
