...
const SideBarOption = ({ path, label, icon: Icon, sidebarExpanded, pathname,setSideBarOpen }) => {
  return (
    <li onClick={()=>setSideBarOpen(false)}  className={`p-5 last:mb-0 ${pathname === path ? "bg-white " : ""}`}>
      ...
      >
        <Icon size={20} className={`flex-shrink-0 ${sidebarExpanded ? "ml-0" :"ml-3" }`} color={`${pathname === path ? "#223E66" : "white"}`} />
        <span className={`ml-2 text-sm font-medium ${sidebarExpanded ? "opacity-100" : "opacity-0"} transition-opacity duration-200`}>
          {label}
        </span>
      </NavLink>
    </li>
